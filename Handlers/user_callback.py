...
from Keyboards.user_kb import mask_kb, show_category, boosts_kb, inventory_section_kb
...
async def open_category(callback: CallbackQuery, state: FSMContext, dp_pool, user_id):
    ...
    await show_category(callback.message, category, balance, page=1)
...
async def switch_section(callback: CallbackQuery, dp_pool, user_id, username):
    ...
    text = (f'<a href="tg://user?id={user_id}">{username}</a> открывает свой 🧳Чемодан:\n'
            f'Баланс: {balance} 🪙\n'
            f'Маска: {active_mask}\n\n'
            f'{section_name}')
...
@user_callback.callback_query(F.data.startswith("use_mask:") or F.data.startswith("use_boost:"))
async def use_item(callback: CallbackQuery, dp_pool, username):
